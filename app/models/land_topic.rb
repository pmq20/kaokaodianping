...
class LandTopic
  ...
  belongs_to :node, :class_name => 'LandNode',:foreign_key=>'node_id'
  belongs_to :last_reply_user, :class_name => 'User'
  has_many :replies, :class_name => 'LandReply',:foreign_key=>'topic_id'
  ...
  def user_readed?(user_id)
    uids = Rails.cache.read("LandTopic:user_read:#{self.id}")
    if uids.blank?
  ...
  def user_readed(user_id)
    ...
    if uids.blank?
      ...
      uids = uids.dup
		...
		uids << user_id
    Rails.cache.write("LandTopic:user_read:#{self.id}",uids)
  end

  # 清除用户读过的记录
  # 用户回复的时候清除状态
  def clear_user_readed
    Rails.cache.write("LandTopic:user_read:#{self.id}",nil)
  end
