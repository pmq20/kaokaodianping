# coding: utf-8  
class Bbs::HomeController < BbsController
  ...
  def index
    if !fragment_exist? "home/last_topics"
      @last_topics = LandTopic.recent.limit(10)
    end
    if !fragment_exist? "home/actived_topics"
      @actived_topics = LandTopic.last_actived.limit(10)
    end
  end

	def auth_callback
		auth = request.env["omniauth.auth"]  
		redirect_to bbs_root_path if auth.blank?
      ...
      sign_in @user if !@user.blank?
			redirect_to bbs_root_path, :notice => "登陆成功。"
		else
      ...
      sign_in @user if !@user.blank?
			redirect_to bbs_root_path,:notice => "欢迎来自 #{auth['provider'].titleize} 的用户，你的帐号已经创建成功。"
		end
	end
