...
class UsersController < ApplicationController
  ...
  def init_user
    ...
    @user = User.find_by_slug(params[:id].split('_').join('.')) if @user.blank? or !@user.deleted.blank?
  ...
  def asked_to
    @per_page = 10
    @asks = Ask.normal.asked_to(@user.id).recent
                  .paginate(:page => params[:page], :per_page => @per_page)
    set_seo_meta("问#{@user.name}的点评")
    if params[:format] == "js"
  ...
  def show
    @per_page = 10
    @logs = Log.where(:user_id => @user.id).desc("$natural").paginate(:page => params[:page], :per_page => @per_page)
